...
;; Migration:
;; - Load instances from old repositories
;;   - Connect to existing roots (default oids)
;;   - Prior deserialization
;; - 
;; - Copy all instances of a given class
;;   (can we copy a class w/o one in memory?)
;;   - Indicies are implicitly created
;;   - Associations are implicitly created
...
(defmethod migrate ((dst store-controller) (src store-controller))
  (if (up-to-date-p src)
	  ...
	  (error "Cannot migrate from store ~A with database version ~A"
		 src (database-version src)))))

(defun migrate-from-current (dst src)
  ;; Migrate all persistent instances
  (map-index (lambda (cname schema sid)
	       (declare (ignore sid schema))
	       (unless (or (inhibited-class-copy-p cname)
			   (not (migrate-class-index-p (find-class cname))))
		 (migrate-class-instances dst cname)))
	     (controller-schema-name-index src))
  ;; Migrate all reachable objects
...
(defun migrate-class-instances (dst classname)
  (map-class (lambda (inst) 
	       (migrate dst inst))
	     classname))

(defun inhibited-class-copy-p (classname)
  "Ignore all classes that "
  (some #'(lambda (type)
	    (subtypep classname type))
	'(persistent-collection cursor pset)))

;;
;; LEGACY MIGRATION FROM 091/090/060 DBs
;;
...
(defun migrate-from-legacy (dst src)
  "Perform a wholesale repository migration from the root. 
   ...
   of the same type!"
  ;; Simpler version of copying indexed classes since we have no explicit
  ;; index creation/removal.  Multi-store indexing makes this whole process
  ;; simpler than in 0.9.1
  (map-btree (lambda (classname classidx)
		 ...
		 (if *migrate-verbose*
	       ...
	       (copy-cindex-contents dst classidx))
...
(defun copy-cindex-contents (sc oldidx)
  (let ((count 1))
...
(defun retrieve-copied-object (dst src)
  ...
  (cond (*oid-btree*
	 (let ((record (get-value (oid src) *oid-btree*)))
	   (get-cached-instance dst (car record))))
	(*oid-hash*
	 (gethash (oid src) *oid-hash*))
